...
int HydrologyH_Reset(void)
{
    ...
    ret = Hydrology_WriteStoreInfo(HYDROLOGY_D_FILE_E_INFO, 0, (uint8_t *)Element_table,
            sizeof(Element_table));

    return ret;
}

int HydrologyH_InitSend(uint8_t Count, HydrologyBodyType Funcode)
{
    int i = 0;
...
static void HydrologyH_SetDownHeaderSequence(uint16_t Count, uint16_t Total)
{
...
static void HydrologyH_MakeDownHeader(HydrologyMode Mode, HydrologyBodyType Funcode)
{
    ...
    Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_BA_REMOTE,
        header->remoteaddr, 5);
    header->len += 5;
    Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_BA_CENTER,
        &(header->centeraddr), 1);
    header->len += 1;

    Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_BA_PASSWORD,
        header->password, 2);
    header->len += 2;
    ...
    HydrologyMode Mode, HydrologyBodyType Funcode)
{
    ...
    switch(Funcode)
    {
        ...
        case Period:
            ...
            Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, Element_table[0].Addr,
        ...
        case SetICCard:
            ...
            for(i = 0; i < downbody->count; i++)
            {
                ...
                Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, Element_table[i].Addr,
        ...
        case Pump:
            ...
            {
                Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_PDA_PUMP_LEN,
                ...
                Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_PDA_PUMP,
                    downbody->element[0]->value, downbody->element[0]->num);
        ...
        case Valve:
            ...
            if(Mode == HYDROLOGY_M4)
            {
                Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_PDA_VALVE_LEN,
                    downbody->element[0]->guide, 1);
                downbody->element[0]->num = downbody->element[0]->guide[0];
                downbody->element[0]->value =
                    (uint8_t *) kmalloc(downbody->element[0]->num, __GFP_ZERO);
                ...
                Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_PDA_VALVE,
                    downbody->element[0]->value, downbody->element[0]->num);
                downbody->len += downbody->element[0]->num + 1;
            }
        ...
        case Gate:
            ...
            if(Mode == HYDROLOGY_M4)
            {
                Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_PDA_GATE_LEN,
                    downbody->element[0]->guide, 1);
                ...
                Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_PDA_GATE,
                    downbody->element[0]->value, downbody->element[0]->num);
                downbody->len += downbody->element[0]->num + 1;
            }
        ...
        case WaterSetting:
            ...
            Hydrology_ReadStoreInfo(HYDROLOGY_H_FILE_E_DATA, HYDROLOGY_PDA_WATERSETTING,
                downbody->element[0]->value, downbody->element[0]->num);
    ...
    HydrologyBodyType Funcode, uint8_t End)
{
    ...
    uint16_t pointer;
    uint8_t stime[6] = {0, 0, 0, 0, 0, 0};
    uint8_t ctime[6] = {0, 0, 0, 0, 0, 0};
    HydrologyDownHeader *header = (HydrologyDownHeader *)g_Hydrology.downpacket->header;
    HydrologyDownBody *downbody = (HydrologyDownBody *)g_Hydrology.downpacket->body;

    buffer_size = header->len + downbody->len + 3;
    g_Hydrology.downpacket->buffer = (uint8_t *)kmalloc(buffer_size, __GFP_ZERO);
...
static int HydrologyH_MakeErrDownTailandSend(HydrologyMode Mode,
    HydrologyBodyType Funcode, uint8_t Err_Packet)
{
    uint8_t *buffer;
    uint16_t i;
    uint16_t buffer_size;
...
static int HydrologyH_CheckUpPacket(uint8_t *input, int inputlen)
{
    uint16_t crcRet = 0;
    uint16_t inputCrc = 0;
    uint16_t bodylen = 0;
...
static int HydrologyH_MakeUpHeader(uint8_t *input, int inputlen, int *position, int *bodylen)
{
...
static int HydrologyH_MakeUpBody(uint8_t *input, int len, int position,
    HydrologyMode Mode, HydrologyBodyType Funcode)
{
    uint32_t i, j, offset;
    int32_t tmp_len;
        ...
        case SetICCard:
            for(i = 0; i < upbody->count; ++i)
            {
                ...
                upbody->element[i]->value =
                    (uint8_t *) kmalloc(upbody->element[i]->num, __GFP_ZERO);
...
int HydrologyH_PrintUpPacket(void)
{
    ...
    switch((HydrologyBodyType)header->funcode)
    {
        ...
        case SetICCard:
            ...
            status_val = *((uint32_t *)upbody->element[0]->value);
...
int HydrologyH_ProcessReceieve(uint8_t *input, int inputlen, HydrologyMode Mode)
{
...
void HydrologyH_ProcessEndIdentifier(uint8_t End)
{
    switch(End)
    {
        case ETX:
...
int HydrologyH_ProcessM3ErrPacket(HydrologyElementInfo *Element_table, uint8_t Count,
    HydrologyBodyType Funcode, uint8_t CErr, uint16_t Err_Packet)
{
    uint8_t **Data;
    uint16_t Len;
...
int HydrologyH_ProcessM4(HydrologyElementInfo *Element_table, uint8_t Count,
...
int HydrologyH_Process(HydrologyElementInfo *Element_table, uint8_t Count,
    HydrologyMode Mode, HydrologyBodyType Funcode)
{
    uint8_t ret = false;
