		  ...
		  const char *fmt)
{
	...
	for (i = 0; i < depth; i++, d = p) {
		...
		array[i] = (char *)READ_ONCE(d->d_name.name);
		if (p == d) {
			if (i)
				array[i] = "";
			i++;
...
//	}
		...
		buf = string(buf, end, "fwnode prefix",
			     default_str_spec);
		buf = string(buf, end, "fwnode name",
			     default_str_spec);
		    ...
		    struct printf_spec spec, const char *fmt)
{
	...
	switch (*fmt) {
	case 'P':	/* name */
		buf = string(buf, end, "fwnode name", str_spec);
		break;
	case 'f':	/* full_name */
	default:
