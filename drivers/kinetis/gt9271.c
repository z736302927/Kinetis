...
static inline void gt9271_Delayus(u32 ticks)
{
    udelay(ticks);
}

static inline void gt9271_Delayms(u32 ticks)
{
    mdelay(ticks);
}
...
static int8_t GTP_I2C_Test(void)
{
    ...
    gt9271_port_multi_receive(GTP_REG_CONFIG_DATA, Data, sizeof(Data));
...
static void GTP_Touch_Down(int32_t id, int32_t x, int32_t y, int32_t w)
{
    ...
    /*取x、y初始值大于屏幕像素值*/
    kinetis_debug_trace(KERN_DEBUG, "ID:%d, X:%d, Y:%d, W:%d", id, x, y, w);
...
static void GTP_Touch_Up(int32_t id)
{
    ...
    kinetis_debug_trace(KERN_DEBUG, "Touch id[%2d] release!", id);
}
...
static void Goodix_TS_Work_Func(void)
{
    ...
    gt9271_port_multi_receive(GTP_READ_COOR_ADDR, point_data, 10);
    ...
    if (touch_num > 1) { //不止一个点
        ...
        gt9271_port_multi_receive(GTP_READ_COOR_ADDR, buf, 8 * (touch_num - 1));
...
exit_work_func: {
        gt9271_port_multi_transmmit(GTP_READ_COOR_ADDR, end_cmd, 1);
    }
}
...
int8_t GTP_Reset_Guitar(void)
{
    ...
    gt9271_port_multi_transmmit(GTP_REG_COMMAND, 0x02, 1);
...
//  gt9271_port_multi_transmmit(GTP_REG_COMMENT, 0x05, 1);

    return ret;
}


//int8_t GTP_Send_Command(u8 command)
//{
//  int8_t ret = -1;
//
//  gt9271_port_multi_transmmit(GTP_REG_COMMAND, GTP_COMMAND_READSTATUS, 1);
//
//  return ret;
//}
...
static int32_t GTP_Get_Info(void)
{
    ...
    gt9271_port_multi_receive(GTP_REG_CONFIG_DATA + 1, opr_buf, 4);

    abs_x_max = (opr_buf[1] << 8) + opr_buf[0];
    abs_y_max = (opr_buf[3] << 8) + opr_buf[2];

    gt9271_port_multi_receive(GTP_REG_CONFIG_DATA + 6, opr_buf, 1);

    int_trigger_type = opr_buf[0] & 0x03;

    kinetis_debug_trace(KERN_DEBUG, "X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x", abs_x_max, abs_y_max, int_trigger_type);
...
int32_t GTP_Read_Version(void)
{
    ...
    gt9271_port_multi_receive(GTP_REG_VERSION, version, 4);
...
int32_t GTP_Init_Panel(void)
{
    ...
    if (GTP_Read_Version() == GT9271_ID) {
        temp = 0x02;
        gt9271_port_multi_transmmit(GT9271_CTRL_REG, &temp, 1);
        gt9271_port_multi_receive(GT9271_CFGS_REG, config_data, 1);
        ...
        if (config_data[0] < 0x41) {
            ...
            gt9271_port_multi_transmmit(GTP_REG_CONFIG_DATA, CTP_CFG_GT9271, sizeof(CTP_CFG_GT9271));
            gt9271_port_multi_transmmit(GT9271_CHECK_REG, config_data, 2);
        }
...
//  u8 buf[200],j;
//
//  ret = gt9271_port_multi_receive(GT9271_CFGS_REG, buf, sizeof(CTP_CFG_GT9271));
//
//  for(i = 0;i < sizeof(CTP_CFG_GT9271);i++)
//  {
//    if(CTP_CFG_GT9271[i] != buf[i])
//    {
////   kinetis_debug_trace(KERN_DEBUG, "Config fail ! i = %d ",i);
////      return -1;
//      j++;
//    }
//  }
//kinetis_debug_trace(KERN_DEBUG, "Config success ! i = %d ",j);
