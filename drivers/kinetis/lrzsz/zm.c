...
zm_get_hex_encoded_byte(zm_t *zm)
{
	...
	c += (n << 4);
	pr_notice("zm_get_hex_encoded_byte: %02X", c);
	return c;
}
...
again2:
	...
	pr_debug(_("Bad escape sequence %x"), c);
	return ERROR;
}
...
zm_put_escaped_char(zm_t *zm, int c)
{
	...
	case ZM_ESCAPE_NEVER:
		serial_port_transmit_byte(zm->serial, zm->lastsent = c);
		break;
	case ZM_ESCAPE_ALWAYS:
		serial_port_transmit_byte(zm->serial, ZDLE);
		/* Spec 7.2: The receiving program decodes any sequence
		 * of ZDLE followed by a byte with bit 6 set and bit 5 reset
		 * (uppercase letter, either parity) to the equivalent control
		 * character by inverting bit 6 */
...
zm_send_binary_header(zm_t *zm, int type)
{
	...
	pr_notice("zm_send_binary_header: %s %lx", frametypes[type + FTOFFSET], zm_reclaim_send_header(zm));
	if (type == ZDATA)
		for (int n = 0; n < zm->znulls; n ++) {
			serial_port_transmit_byte(zm->serial, 0);
		}

	serial_port_transmit_byte(zm->serial, ZPAD);
	serial_port_transmit_byte(zm->serial, ZDLE);
	...
	if (zm->crc32t) {
		...
		/* Spec 7.3.1. A binary header begins with the sequence
		   ZPAD, ZDLE, ZBIN. */
		serial_port_transmit_byte(zm->serial, ZBIN);
...
zm_send_binary_header32(zm_t *zm, int type)
{
	 ...
	 * character. */
	serial_port_transmit_byte(zm->serial, ZBIN32);
...
zm_send_hex_header(zm_t *zm, int type)
{
	...
	pr_notice("zm_send_hex_header: %s %lx", frametypes[(type & 0x7f) + FTOFFSET], zm_reclaim_send_header(zm));
	...
	serial_port_transmit_bytes(zm->serial, (const u8 *)s, len);
}
...
zm_send_data(zm_t *zm, const char *buf, size_t length, int frameend)
{
	...
	pr_notice("zm_send_data: %lu %s", (unsigned long) length,
	...
	serial_port_transmit_byte(zm->serial, ZDLE);
	serial_port_transmit_byte(zm->serial, frameend);
	...
	zm_put_escaped_char(zm, crc);
	if (frameend == ZCRCW) {
		serial_port_transmit_byte(zm->serial, XON);
	}
}
...
zm_send_data32(zm_t *zm, const char *buf, size_t length, int frameend)
{
	int c;
	unsigned long crc;
	pr_notice("zsdat32: %zu %s", length, Zendnames[(frameend - ZCRCE) & 3]);
...
crcfoo:
			...
			case GOTCRCW: {
				...
				*bytes_received = i;
				...
				pr_notice("zm_receive_data: %lu  %s", (unsigned long)(*bytes_received),
					Zendnames[(d - GOTCRCE) & 3]);
				return d;
			}
			case GOTCAN:
				pr_err(_("Sender Canceled"));
				return ZCAN;
			case TIMEOUT:
				pr_err(_("TIMEOUT"));
				return c;
			default:
				pr_err(_("Bad data subpacket"));
				return c;
			}
		}
		buf[i] = c;
	...
	pr_err(_("Data subpacket too long"));
	return ERROR;
}
...
crcfoo:
			...
			case GOTCRCW:
				...
				*bytes_received = i;
				COUNT_BLK(*bytes_received);
				pr_notice("zm_read_data32: %lu %s", (unsigned long) *bytes_received,
					Zendnames[(d - GOTCRCE) & 3]);
				return d;
			case GOTCAN:
				pr_err(_("Sender Canceled"));
 ...
 */
int
zm_get_header(zm_t *zm, u32 *payload)
{
	int c, cancount;
...
agn2:
		if (intro_msg_len > max_intro_msg_len) {
			pr_err(_("Intro message length exceeded"));
			return (ERROR);
		}
...
fifi:
	...
	case TIMEOUT:
	case RCDO:
		pr_err(_("Got %s"), frametypes[c + FTOFFSET]);
	/* **** FALL THRU TO **** */
	default:
		if (c >= -3 && c <= FRTYPES) {
			pr_notice("zm_get_header: %s %lx", frametypes[c + FTOFFSET], (unsigned long) rxpos);
		} else {
			pr_notice("zm_get_header: %d %lx", c, (unsigned long) rxpos);
		}
	}
	if (payload) {
		*payload = rxpos;
	...
	if (intro_msg_len > 0) {
		pr_info("zm_get_header: received intro msg from sender...");
		pr_info("%s", intro_msg);
	}
	kfree(intro_msg);
...
zm_read_binary_header32(zm_t *zm)
{
	...
	crc = UPDC32(c, crc);
#ifdef DEBUGZ
	pr_notice("zm_read_binary_header32 c=%X  crc=%lX", c, crc);
#endif
...
/* Store pos in Txhdr */
void
zm_set_header_payload(zm_t *zm, u32 val)
{
	zm->Txhdr[ZP0] = val;
	zm->Txhdr[ZP1] = val >> 8;
	zm->Txhdr[ZP2] = val >> 16;
...
void
zm_set_header_payload_bytes(zm_t *zm, u8 x0, u8 x1, u8 x2, u8 x3)
{
	zm->Txhdr[ZP0] = x0;
	zm->Txhdr[ZP1] = x1;
	zm->Txhdr[ZP2] = x2;
	...
	for (;;) {
		...
		case ZFIN:
			/* Spec 8.3: "When the sender receives the
			             ...
			             * invoked it." */
			serial_port_transmit_byte(zm->serial, 'O');
			serial_port_transmit_byte(zm->serial, 'O');
		case ZCAN:
		case TIMEOUT:
			return;
 ...
 */
int
zm_do_crc_check(zm_t *zm, FIL *f, size_t remote_bytes, size_t check_bytes)
{
	unsigned long crc;
	unsigned long rcrc;
	size_t n;
	...
	if (check_bytes == 0 && ((size_t) f_size(f)) != remote_bytes) {
	...
	while (n > 0) {
		to_read = (n > sizeof(buf)) ? sizeof(buf) : n;
		ret = f_read(f, buf, to_read, &br);
		if (ret != FR_OK || br == 0) {
			break;
		}
		for (UINT i = 0; i < br; i++) {
