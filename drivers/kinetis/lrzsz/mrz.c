...
int t_mrz_receive_specified_file(int argc, char *argv[])
{
	struct serial_port *serial;
	int ret = 0;
	...
	size_t bytes = zmodem_receive(serial, LRZSZ_RZ_DIRRECTORY,
			approver_cb,
			tick_cb,
			complete_cb,
			0,
...
static void *lrzsz_sz_sim_thread(void *data)
{
	struct sz_thread_arg *arg = (struct sz_thread_arg *)data;

	zmodem_send(arg->serial, arg->n_filenames, arg->filenames,
			NULL, NULL, 0, RZSZ_FLAGS_NONE);

	return NULL;
}

int t_mrz_receive_random_file(int argc, char *argv[])
{
	struct serial_port *serial_sz, *serial_rz;
	struct sz_thread_arg sz_arg;
	pthread_t thread;
	int ret;
	int n_filenames = 0;
	...
	sz_arg.serial = serial_sz;
	sz_arg.n_filenames = n_filenames;
	sz_arg.filenames = filenames_sz;
		
	ret = pthread_create(&thread, NULL, lrzsz_sz_sim_thread, &sz_arg);
	if (ret != 0) {
		pr_err("failed to create sz thread: %d\n", ret);
		goto thread_err;
	}

	/* Receive files */
	size_t bytes = zmodem_receive(serial_rz, LRZSZ_RZ_DIRRECTORY,
			approver_cb,
			tick_cb,
			complete_cb,
			0,
			RZSZ_FLAGS_NONE);
	pr_info("Received %zu bytes in %d files.\n", bytes, n_filenames);
