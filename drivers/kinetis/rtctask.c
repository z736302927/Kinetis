...
/**
  ...
  * @retval None
  */
static u8 RTCTask_Expired(RTCTask_TypeDef *Target)
{
    u8 tmp_timecmp1[12];
    u8 tmp_timecmp2[12];
    int8_t tmp_res = 0;
...
static void RTCTask_Time_AddSecond(RTCTask_DateTime_TypeDef *TmpCalendar, u8 Seconds)
{
    if (Seconds > 60)
        return ;
...
static void RTCTask_Time_AddMinute(RTCTask_DateTime_TypeDef *TmpCalendar, u8 Minutes)// Minutes cannot be greater than 60 minutes
{
    if (Minutes > 60)
        return ;
...
static void RTCTask_Time_AddHour(RTCTask_DateTime_TypeDef *TmpCalendar, u8 Hours)// An hour cannot be greater than 24
{
    if (Hours > 24)
        return;
...
//static void RTCTask_Time_AddDay(RTCTask_DateTime_TypeDef *TmpCalendar, u8 Days)
//{
...
//static void RTCTask_Time_AddMonth(RTCTask_DateTime_TypeDef *TmpCalendar, u8 Months)
//{
//    if(Months > 12)
//        return;
...
static u8 RTCTask_Flag = 0;

void RTCTask1_Callback(void)
{
    RTCTask_Flag = true;
    printk(KERN_DEBUG "RTCTask1 timeout!");
}
...
int t_RTCTask_Add(int argc, char **argv)
{
    ...
    printk(KERN_DEBUG "RTCTask elapse time = %lu ms.", Timestamp);
