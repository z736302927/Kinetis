...
void USBD_ResetEP(uint32_t EPNum)
{
    ...
    if (EPNum & 0x80) {
        ...
        if (EP_IN_TYPE(EPNum) == USB_ENDPOINT_TYPE_ISOCHRONOUS) {
            if (USBD_GetFrame() & 1)
                DIEPCTL(EPNum) |= (1 << 28); /* set even frame */
            else
                DIEPCTL(EPNum) |= (1 << 29);                     /* set odd frame  */
...
uint32_t USBD_ReadEP(uint32_t EPNum, uint8_t *pData)
{
    ...
    if ((DOEPCTL(EPNum) & (1 << 15)) == 0)
        return (0); /* if Ep not active      */
    ...
    if (isoEpFlag) {
        ...
        if ((DOEPTSIZ(EPNum) & ((0x3FF << 19) | (3 << 29))) == 0)
            ...
            IsoOutPacketDataCnt[EPNum] = 0;/* else data is unvalid               */
    }
...
uint32_t USBD_WriteEP(uint32_t EPNum, uint8_t *pData, uint32_t cnt)
{
    ...
    if (!SyncWriteEP && InPacketDataPtr[EPNum]) {
        ...
        if (cnt)
            while ((DTXFSTS(EPNum) * 4) < cnt); /* get space in Ep TxFIFO    */
        ...
        if (isoEpFlag) {                    /* if Isochronous Ep: set packet frame*/
            if (USBD_GetFrame() & 1)
                DIEPCTL(EPNum) |= (1 << 28); /* even frame     */
            else
                DIEPCTL(EPNum) |= (1 << 29);                     /* odd frame      */
        }
        ...
        while (msk) {
            ...
            if (DOEPINT(num) & (1 << 1)) {
                if (EP_OUT_TYPE(num) == USB_ENDPOINT_TYPE_ISOCHRONOUS) {
                    ...
                    if ((USBD_GetFrame() & 1))
                        DOEPCTL(num) |= (1 << 28); /* even frame */
                    else
                        DOEPCTL(num) |= (1 << 29); /* odd frame  */
