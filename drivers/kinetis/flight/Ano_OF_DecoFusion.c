...
static void ANO_OF_Fusion(u8 *dT_ms, s32 ref_height_cm)
{
    ...
    //
    if (UPOF_UP_DW == 0) {
        of_rdf.of_ref_height = clamp(ref_height_cm, 20, 500); //限制到20cm-500cm
    } else {
        of_rdf.of_ref_height = clamp((OBJREF_HEIGHT_CM - ref_height_cm), 20, 500); //限制到20cm-500cm
    }
    ...
    case 1: {
        ...
        if (of_data.valid == 0xf5) {
            ...
            for (u8 i = 0; i < 2; i++) {
                ...
                of_fus_err_i[i] = clamp(of_fus_err_i[i], -100, 100);
                //
