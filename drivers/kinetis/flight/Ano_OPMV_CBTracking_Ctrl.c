...
//
#define OF_DATA_SOURCE               ((kinetis_fmu->of_ok)?1:0)
...
static void ANO_CBTracking_Decoupling(u8 *dT_ms, float rol_degs, float pit_degs)
{
    ...
    if (opmv.cb.sta != 0) {
        ...
        ano_opmv_cbt_ctrl.rp2pixel_val[0] = clamp(ano_opmv_cbt_ctrl.rp2pixel_val[0], -60, 60); //高度120pixel
        ano_opmv_cbt_ctrl.rp2pixel_val[1] = clamp(ano_opmv_cbt_ctrl.rp2pixel_val[1], -80, 80); //宽度160pixel
