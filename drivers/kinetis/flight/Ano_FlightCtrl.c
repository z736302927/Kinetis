...
void Swtich_State_Task(u8 dT_ms)
{
    ...
    //
    kinetis_fmu->of_ok = (ano_of.work_sta * ano_of.link_sta);

    //光流模块
    if (kinetis_fmu->of_ok || kinetis_fmu->of_df_ok) {
        //
        if (kinetis_fmu->of_ok) {
            jsdata.of_qua = ano_of.of_quality;
            jsdata.of_alt = (u16)ano_of.of_alt_cm;
        } else if (kinetis_fmu->of_df_ok) {
