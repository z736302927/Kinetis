...
// 		*zoom_adj += ( 1 / ( 1 + 1 / ( hz *6.28f *T ) ) ) *clamp( (x - *zoom_adj),(- zoom),(zoom) );
...
/*
va x vb = |va||vb| *sinx *vn,vn为垂直于va,vb的单位向�?
归一化计算，�?va x vb = sinx;
*/

float vec_2_cross_product(float in1[2], float in2[2]) //正负为in1->in2 夹角逆时�?
{
...
void vec_3_cross_product_err_sinx(float in1[3], float in2[3], float out[3]) //输出xyz误差夹角x 的sin(x)，右手螺�?
{
