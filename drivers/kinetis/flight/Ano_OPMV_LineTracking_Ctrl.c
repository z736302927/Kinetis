...
static void ANO_LTracking_Decoupling(u8 *dT_ms, float rol_degs, float pit_degs)
{
    ...
    if (opmv.lt.sta != 0) {
        ...
        ano_opmv_lt_ctrl.r2pixel_val = clamp(ano_opmv_lt_ctrl.r2pixel_val, -80, 80); //宽度160pixel
...
void ANO_LTracking_Ctrl(u8 *dT_ms, u8 en)
{
    ...
    if (en) {
        ...
        if (opmv.lt.sta == 1) {
            ...
            ano_opmv_lt_ctrl.exp_yaw_pal_dps = clamp(ano_opmv_lt_ctrl.exp_yaw_pal_dps, -100, 100);
        } else
            ano_opmv_lt_ctrl.exp_yaw_pal_dps = 0;
