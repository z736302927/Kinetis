...
void Thermostatic_Ctrl_Task(u8 dT_ms)
{
    ...
    if (flag.unlock_sta == 0) {
        if (Ano_Parame.set.heat_switch == 1) { //开启恒温功能
...
//				sensor.gyr_CALIBRATE = 2;//重新校准陀螺仪
    ...
    if (thermostatic_en) {
        ...
        if (1) { //((temperature_ctrl_val)<100)
            //积分偏差限幅
            temperature_err_i += clamp(temperature_err, -10, 10) * dT_ms * 0.001f;
            //积分限幅
            temperature_err_i = clamp(temperature_err_i, -20, 20);
        }
        ...
        //
        temperature_ctrl_val = clamp(temperature_ctrl_val, 0, 100);
        //恒温控制量输出
