...
void Fc_Sensor_Get()//1ms
{
    ...
    if (flag.start_ok) {
        ...
        if (cnt == 0) {
            /*读取电子罗盘磁力计数据*/
            if (kinetis_fmu->mag_type == MAG_AK8975)
                DrvAk8975Read();
            else if (kinetis_fmu->mag_type == MAG_AK09915)
                DrvAk09915Read();
...
////////////////////////////////////////////////////////////////////////
    ...
    imu_state.M_fix_en = kinetis_fmu->mag_ok;		//磁力计修正使能
...
{
    if (kinetis_fmu->mag_type == MAG_AK8975) //8975
        Mag_Get(mag_val);
    else if (kinetis_fmu->mag_type == MAG_AK09915) { //09915
...
//void Baro_Get_Task()
//{
////			ref_height_get += clamp((s32)user_spl0601_get() - ref_height_get,-20,20 );
...
void WCZ_Fus_Task(u8 dT_ms)
{
    ...
    if ((kinetis_fmu->of_df_ok || kinetis_fmu->of_ok) && baro_offset_ok) { //TOF或者OF硬件正常，且气压计记录相对值以后
