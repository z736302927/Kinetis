...
//BC95_B5 AT instruction description supported
...
typedef struct {
    u8 nb_socket_id;     //指示相应的socket id
    u16 nb_data_len;     //提示数据长度
} udp_rece_t;
//******************************************************************************
// Name : nb95_status_t
//
// brief : 定义存储BC95的状态信息
typedef struct {
    u8      nb95_connection_status;
    u8      nb95_register_status;
    u8      nb95_IMSI[16];
    u8      nb95_IMEI[16];
...
//==============================================================================
//用于保存当前正在执行的AT指令
static NB_CmdInfo g_at_cmd;
//==============================================================================
//NBiot 操作状态
static  NB_ModuleState        g_nb_state = {PROCESS_NONE, 0};
...
// bc95 初始化流程
//
const u8 nb95_init_process[] = {SUB_NONE, SUB_SYNC, SUB_CMEE, SUB_CFUN,
...
// bc95 获取信息流程
//
const u8 nb95_info_process[] = {SUB_NONE, SUB_CGMI, SUB_CGMM, SUB_CGMR,
        SUB_NBAND, SUB_END
    };
...
// bc95 UDP创建流程
//
const u8 nb95_udpcr_process[] = {SUB_NONE, SUB_UDP_CR, SUB_UDP_CL, SUB_UDP_CR,
        SUB_END
    };

// bc95 UDP关闭流程
const u8 nb95_udpcl_process[] = {SUB_NONE, SUB_UDP_CL, SUB_END};

// bc95 UDP发送流程
const u8 nb95_udpst_process[] = {SUB_NONE, SUB_UDP_ST, SUB_END};

//bc95 UDP
//const u8 nb95_udpre_process[] = {SUB_NONE,UB_UDP_RE,SUB_END};
//==============================================================================
// 函数
//==============================================================================
...
static u32 NB_Strtoul(const char *pStr, int base);
static u16 NB_HexStrToNum(char *str);
...
static void NB_SendCmd(HWAttrs_Handle hw_handle, CmdHandle cmdHandle)
{
    ...
    hw_handle->Uart->Send((u8 *)gNBSendBuf.Buf, strLen);
...
u8 addr_adjust(char *buf, char *pStart, u16 *plen)
{
    u8 isAsync = false;
    char *pEnd = NULL;
    u8 msg_len = 0;
...
u8 bc95_AsyncNotification(char *buf, u16 *len)
{
    u8 isAsync = false;
...
//         len -> 数据长度
// return : none
static void bc95_receCb(char *buf, u16 len)
{
    if (len == 0)
        return;
...
int bc95_open(NB_Handle handle)
{
    ...
    //注册串口接收回调函数，
    hw_handle->Uart->Open(bc95_receCb, hw_handle->Baudrate);
...
int bc95_sendUDP(NB_Handle handle, int len, char *msg)
{
    ...
    u16 max_len = (NB_UART_SEND_BUF_MAX_LEN - 40) >> 1;
    u16 str_len = 0;
    ...
    u16 msg_len = snprintf(buf, NB_UART_SEND_BUF_MAX_LEN - 40, "%s,%s,%s,%d,",
            g_bc95_status.nb95_udp_id,
            REMOTE_SERVER_IP,
            REMOTE_SERVER_PORT,
            str_len);

    for (u16 i = 0 ; i < str_len ; i++)
        sprintf(&buf[msg_len + (i << 1)], "%02X", (u8)msg[i]);
...
int bc95_coapSendMsg(NB_Handle handle, int len, char *msg)
{
    ...
    u16 msg_len = snprintf(buf, NB_UART_SEND_BUF_MAX_LEN - 40, "%d,", str_len);
    ...
    if (isOk) {
        ...
        if (g_at_cmd.cmd_action == ACTION_OK_NEXT_ERROR_TRY) {
            ...
            if (g_at_cmd.haveTried < g_at_cmd.cmd_try) {
                hw_handle->Uart->Send((u8 *)gNBSendBuf.Buf, gNBSendBuf.len);
...
int bc95_main(NB_Handle handle)
{
    ...
    if (g_event_regTable & NB_SP_RECE_EVENT) {
        ...
        if ((isPass >= false) && (isPass <= true)) {
            ...
            while ((param[index] = strsep(&tmp_buf, "\r\n")) != NULL) {
                index++;
                tmp_buf = NULL;
...
void nbsend_msg_app(NB_Handle handle, char **buf, bool isOk)
{
    ...
    if (g_nb_state.state == PROCESS_INIT) {
        switch (nb95_init_process[g_nb_state.sub_state]) {
            ...
            case SUB_CGSN: {
                ...
                if (pColon) {
                    ...
                    handle->AppReceiveCallback((NB_MessageTypeDef)TYPES_CGSN, 15, (char *)g_bc95_status.nb95_IMEI);
                }
            }
            break;
        ...
        switch (nb95_info_process[g_nb_state.sub_state]) {
            ...
            case SUB_NBAND: {
                ...
                if (pColon) {
                    ...
                    handle->AppReceiveCallback((NB_MessageTypeDef)TYPES_NBAND, strlen(pFreq), pFreq);
                }
            }
            break;
        ...
        if (g_nb_state.sub_state == 1) {
            char *pColon = strchr(buf[0], ':');
            pColon++;
            u8 lqi = NB_Strtoul(pColon, 10);
            int8_t rssi = -113 + (lqi << 1);
            u8 len = snprintf(buf[0], 10, "%d", rssi);
            *(buf[0] + len) = 0;
        ...
        if (g_nb_state.sub_state == 1) {
            ...
            while ((param[index] = strsep(&tmp_buf, ",")) != NULL) {
                index++;
                tmp_buf = NULL;
...
{
    return simple_strtoul(pStr, 0, base);
}
