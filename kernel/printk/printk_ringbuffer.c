...
static bool data_push_tail(struct printk_ringbuffer *rb, unsigned long lpos)
{
	...
	while ((lpos - tail_lpos) - 1 < DATA_SIZE(data_ring)) {
		 ...
		 * data_push_tail:A.
		 */
		if (atomic_long_try_cmpxchg(&data_ring->tail_lpos, (long *)&tail_lpos,
...
static bool desc_reserve(struct printk_ringbuffer *rb, unsigned long *id_out)
{
	...
	do {
		 ...
		 *    _prb_commit:B.
		 */
	} while (!atomic_long_try_cmpxchg(&desc_ring->head_id, (long *)&head_id,
	 ...
	 */
	if (!atomic_long_try_cmpxchg(&desc->state_var, (long *)&prev_state_val,
			...
			struct prb_data_blk_lpos *blk_lpos, unsigned long id)
{
	...
	do {
		 ...
		 */
	} while (!atomic_long_try_cmpxchg(&data_ring->head_lpos, (long *)&begin_lpos,
			  ...
			  struct prb_data_blk_lpos *blk_lpos, unsigned long id)
{
	...
	if (!atomic_long_try_cmpxchg(&data_ring->head_lpos, (long *)&head_lpos,
					 ...
					 u32 caller_id, unsigned long *id_out)
{
	 ...
	 */
	if (!atomic_long_try_cmpxchg(&d->state_var, (long *)&prev_state_val,
