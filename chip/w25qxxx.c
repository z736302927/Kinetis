...
void w25qxxx_ReadInfo(uint8_t w25qxxx)
{
    ...
    kinetis_debug_trace(KERN_DEBUG, "g_w25qxxx is 0x%02X.", w25qxxx);
    w25qxxx_ReadJEDECID(w25qxxx, JEDECID);
    kinetis_debug_trace(KERN_DEBUG, "JEDEC ID is 0x%02X%02X%02X", JEDECID[0], JEDECID[1], JEDECID[2]);
    w25qxxx_ReadManufacturer_DeviceID(w25qxxx, &ManufacturerID, &DeviceID);
    kinetis_debug_trace(KERN_DEBUG, "Manufacturer ID is 0x%02X, Device ID is 0x%02X.", ManufacturerID, DeviceID);
    w25qxxx_ReadUniqueIDNumber(w25qxxx, UniqueID);
    kinetis_debug_trace(KERN_DEBUG, "Unique ID is %02X%02X%02X%02X%02X%02X%02X%02X", UniqueID[0], UniqueID[1], UniqueID[2], UniqueID[3], \
...
//    kinetis_debug_trace(KERN_DEBUG, "Tx_Buffer[%d] = 0x%02X, Tx_Buffer[%d] = 0x%02X, Tx_Buffer[%d] = 0x%02X, Tx_Buffer[%d] = 0x%02X,",\
//                   i + 0, Tx_Buffer[i + 0],\
//                   i + 1, Tx_Buffer[i + 1],\
//                   i + 2, Tx_Buffer[i + 2],\
//                   i + 3, Tx_Buffer[i + 3]);
