...
{
    IIC_PortReceive(IIC_1, MPU6050_ADDR, Addr, pData);
}

void mpu6050_PortMultiTransmmit(uint8_t Addr, uint8_t *pData, uint32_t Length)
{
    IIC_PortMultiTransmmit(IIC_1, MPU6050_ADDR, Addr, pData, Length);
}

void mpu6050_PortMultiReceive(uint8_t Addr, uint8_t *pData, uint32_t Length)
{
    IIC_PortMultiReceive(IIC_1, MPU6050_ADDR, Addr, pData, Length);
}
...
void mpu6050_Test(void)
{
    ...
    kinetis_debug_trace(KERN_DEBUG, "mpu6050 Read and write TEST PASSED !");
}
