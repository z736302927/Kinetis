...
def format_filename(filename):
    ...
    Returns:
        str: Formatted filename with extension preserved
    """
    # Split filename and extension
    name_parts = filename.rsplit('.', 1)
    if len(name_parts) == 2:
        name_only, extension = name_parts[0], '.' + name_parts[1]
    else:
        name_only, extension = filename, ''

    # Use same formatting logic as identifiers
    formatted_name = to_linux_style(name_only)
...
def format_filenames_in_project(project_root, dry_run=False):
    """
    Format all source filenames in the project to Linux style

    Args:
        project_root: Project root directory
        dry_run: Whether to only show changes without executing

    Returns:
        tuple: (total_files, renamed_files, rename_details)
    """
    ...
    for file_path in source_files:
        ...
        if original_filename == formatted_filename:
            continue

        # Calculate new full path
        new_path = path_obj.parent / formatted_filename

        # Check if target file already exists in the same directory
        # On Windows, need case-sensitive comparison since Path is case-insensitive
        target_exists = False
        try:
            # List all files in parent directory and do case-sensitive comparison
            if new_path.parent.exists():
                existing_files = [f.name for f in new_path.parent.iterdir()]
                target_exists = formatted_filename in existing_files
        except Exception:
            # Fallback to standard check
            target_exists = new_path.exists()

        if target_exists and str(new_path) != str(path_obj):
            # Target file exists in the same directory - this is a conflict
            try:
                existing_size = new_path.stat().st_size
                source_size = path_obj.stat().st_size

                if existing_size == source_size:
                    # Same size - likely duplicates
                    # Compare modification times
                    source_mtime = path_obj.stat().st_mtime
                    existing_mtime = new_path.stat().st_mtime

                    if source_mtime > existing_mtime:
                        # Source is newer, replace target
                        print(f"  Replacing older target: {formatted_filename}")
                        new_path.unlink()
                    else:
                        # Target is newer or same, skip source
                        print(f"  Skipping {original_filename} (newer target exists: {formatted_filename})")
                        continue
                else:
                    # Different sizes - real conflict, skip to avoid data loss
                    print(f"  Warning: Conflict - {formatted_filename} exists with different size, skipping {original_filename}")
                    continue
            except Exception as e:
                print(f"  Warning: Cannot check conflict for {formatted_filename}, skipping {original_filename}: {str(e)}")
                continue
    ...
    if not dry_run:
        ...
        for detail in rename_details:
            try:
                ...
                orig_path = Path(detail['original'])
                try:
                    rel_path = orig_path.relative_to(Path(project_root))
                    print(f"  Renamed: {rel_path / detail['old_name']} -> {detail['new_name']}")
                except ValueError:
                    # Cannot calculate relative path, use full path
                    print(f"  Renamed: {detail['old_name']} -> {detail['new_name']}")
                renamed_files += 1

                # Update include statements in all source files
                update_include_statements(project_root, detail['old_name'], detail['new_name'])
            except Exception as e:
                print(f"  Error renaming {detail['old_name']}: {str(e)}")
    else:
        print(f"\nDry run - would rename {len(rename_details)} files:")
        for detail in rename_details:
            print(f"  {detail['old_name']} -> {detail['new_name']}")

    return total_files, len(rename_details) if dry_run else renamed_files, rename_details


def update_include_statements(project_root, old_filename, new_filename):
    """
    Update #include statements in all source files after filename rename

    Args:
        project_root: Project root directory
        old_filename: Old filename (with extension)
        new_filename: New filename (with extension)
    """
    # Find all source files
    source_files = find_source_files(project_root)

    # Patterns to match include statements
    # Matches: #include "filename" and #include <filename>
    include_pattern1 = re.compile(r'#include\s*"' + re.escape(old_filename) + r'"')
    include_pattern2 = re.compile(r'#include\s*<' + re.escape(old_filename) + r'>')
    ...
    for file_path in source_files:
        try:
            ...
            # Check if file contains the old filename in include statements
            if include_pattern1.search(content) or include_pattern2.search(content):
                # Replace include statements
                new_content = include_pattern1.sub(f'#include "{new_filename}"', content)
                new_content = include_pattern2.sub(f'#include <{new_filename}>', new_content)
            ...
            print(f"    Error updating includes in {file_path}: {str(e)}")
...
def move_trailing_comments(file_path):
    ...
    try:
        ...
        if lines is None:
            print(
                f"  Unable to read file {file_path}: failed with multiple encodings")
            return False
        ...
        for line in lines:
            ...
            if comment_pos > 0:  # Ensure it's not a leading comment
                ...
                is_assignment = re.match(
                    r'^\s*[\w\*\s\[\].*&]+.*[=]', before_comment)
                # Match control statements: if, while, for, switch, else, do, return, goto, break, continue
                is_control = re.match(
                    r'^\s*(if|while|for|switch|else|do|return|goto|break|continue|case|default)\b', before_comment)
                ...
                is_declaration = re.match(
                    r'^\s*(?:[a-zA-Z_][a-zA-Z0-9_]*\s+)+[\w\*\s\[\].*&\s,]+', before_comment)
            ...
            if changes_made:
                ...
                try:
                    ...
                    # Skip utf-8, try other encodings
        ...
        print(
            f"  Error processing trailing comments for {file_path}: {str(e)}")
        return False
...
def find_identifiers_in_file(file_path):
    ...
    try:
        ...
        typedef_struct_matches = re.findall(
            typedef_struct_pattern, content, re.MULTILINE | re.DOTALL)
        ...
        typedef_struct_contents = re.findall(
            struct_content_pattern, content, re.MULTILINE | re.DOTALL)
        ...
        struct_contents2 = re.findall(
            struct_content_pattern2, content, re.MULTILINE | re.DOTALL)
        ...
        all_brace_contents = re.findall(
            struct_content_pattern3, content, re.MULTILINE | re.DOTALL)
        for struct_content in all_brace_contents:
            ...
            if ';' in struct_content and any(type_name in struct_content for type_name in ['u8', 'u16', 'u32', 'int', 'char', '_dt_frame_st']):
                ...
                array_in_struct_matches = re.findall(
                    array_in_struct_pattern, struct_content)
        ...
        # 匹配类型声明后的变量名（包括复杂的声明）
        var_pattern = types_pattern + \
            r'\s+(?:\*\s*|const\s+|volatile\s+)*(?:\w+\s+)*([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:\[.*?\]|\[[^\]]*\])?\s*[;=,\[\]]'
...
def to_linux_style(name):
    ...
    name_with_underscores = correct_spelling_and_abbreviations(
...
def split_compound_words(name):
    ...
    if '_' not in result and len(result) > 6:
        # 尝试在常见的前缀后插入下划线
        prefixes = ['usb', 'hid', 'uart', 'gpio', 'spi', 'i2c', 'adc', 'dac', 'dma', 'timer', 'counter', 'buffer', 'config', 'status', 'process', 'handle',
                    'update', 'device', 'send', 'recv', 'data', 'value', 'size', 'count', 'length', 'index', 'mode', 'state', 'ctrl', 'reg', 'addr', 'port']
...
def format_identifiers_in_project(project_root, file_path):
    ...
    try:
        ...
        print(
            f"  找到 {total_count} 个需要格式化的标识符（{len(identifiers)} 个变量/函数，{len(macros)} 个宏）")
        ...
        if identifiers:
            ...
            for original_name in sorted_identifiers:
                ...
                # 在整个项目中搜索并替换这个标识符
                success = replace_identifier_in_project(
                    project_root, original_name, linux_name)
                if not success:
        ...
        if macros:
            ...
            for original_macro in sorted_macros:
                ...
                # 在整个项目中搜索并替换这个宏名
                success = replace_macro_in_project(
                    project_root, original_macro, macro_name)
                if not success:
    ...
    try:
        ...
        for source_file in source_files:
            try:
                ...
                # 替换 #define
                define_pattern = r'^(#\s*define\s+)' + \
                    re.escape(old_macro) + r'\b'
                new_content = re.sub(
                    define_pattern, r'\1' + new_macro, new_content, flags=re.MULTILINE)
                if new_content != content:
                    changes_made = True

                # 替换 #ifdef/#ifndef/#if defined()
                ifdef_pattern = r'^(#\s*(?:ifdef|ifndef|if\s+defined)\s*\(\s*)' + \
                    re.escape(old_macro) + r'\s*\)'
                new_content = re.sub(
                    ifdef_pattern, r'\1' + new_macro + r')', new_content, flags=re.MULTILINE)
                if new_content != content:
                    changes_made = True
                ...
                undef_pattern = r'^(#\s*undef\s+)' + \
                    re.escape(old_macro) + r'\b'
                new_content = re.sub(
                    undef_pattern, r'\1' + new_macro, new_content, flags=re.MULTILINE)
                if new_content != content:
                    changes_made = True
                ...
                # 负向 lookahead，后面不能跟 (
                macro_pattern = r'\b' + re.escape(old_macro) + r'(?!\s*\()'
...
def format_directory(directory, astyle_path='astyle', astyle_options=None, dry_run=False):
    ...
    for i, file_path in enumerate(source_files, 1):
        ...
        success, output, error = format_with_astyle(
            file_path, astyle_path, astyle_options)
            ...
            if not dry_run:
                ...
                if identifiers or macros:
                    ...
                    try:
                        ...
                        # 如果出错，使用简单统计
                        estimated_functions = min(len(identifiers), max(
                            1, len(identifiers) // 3))  # 估计至少有一个函数，但不超过总数
                        total_functions += estimated_functions
                        total_variables += len(identifiers) - \
                            estimated_functions

                identifier_success = format_identifiers_in_project(
                    directory, file_path)
                if not identifier_success:
                    print(
                        f"  Warning: Identifier formatting failed for {file_path}")
                    # Continue even if identifier formatting fails

                # Step 4: Second astyle formatting (automatic execution)
                success2, output2, error2 = format_with_astyle(
                    file_path, astyle_path, astyle_options)
                if not success2:
                    print(
                        f"  Warning: Second astyle formatting failed for {file_path}: {error2}")
...
def format_multiple_directories(directories, astyle_path='astyle', astyle_options=None, dry_run=False):
    ...
    for directory in directories:
        ...
        if not os.path.exists(directory):
            print(
                f"Error: Directory '{directory}' does not exist, skipping...")
            continue
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Example usage:
  python format_code.py ./src                    # Format src directory (code + filename)
...
Note: Script execution steps:
      1. Format code with astyle
      2. Auto move trailing comments to line above code
      3. Auto format identifiers to Linux style (lowercase with underscores)
      4. Auto format macros to uppercase style (UPPER_CASE)
      5. Second astyle pass for final cleanup
      6. Format filenames to Linux style (camelCase -> snake_case)
        """
    )
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=True,
        help='Also format filenames to Linux style (lowercase with underscores) [default: enabled]'
    )
    ...
    print(f"Format filenames: {'Yes' if args.format_filenames else 'No'}")
    ...
    # Step 1-5: Execute code formatting for multiple directories
    ...
    # Step 6: Format filenames (if requested) - 默认执行
    ...
    if args.format_filenames:
        for directory in args.directories:
            print(f"\n{'='*60}")
            print(f"Step 6: Processing filenames in: {os.path.abspath(directory)}")
            print(f"{'='*60}")
            ...
            total, renamed, details = format_filenames_in_project(directory, args.dry_run)
            total_filenames_found += total
            total_files_renamed += renamed
            rename_details.extend(details)

        print("\n" + "=" * 60)
        print("Step 6: Filename formatting completed!")
        print(f"Total files found: {total_filenames_found}")
        print(f"Files renamed: {total_files_renamed}")

    print("\n" + "=" * 60)
    print("All formatting steps completed!")
    if args.format_filenames:
        print(f"Step 6 - Filename renames: {total_files_renamed}")
    print(f"Code files processed: {total}")
    ...
    if not args.dry_run:
        ...
        if args.format_filenames:
            print(f"Step 6 - Files renamed: {total_files_renamed}")
        print(
            f"Files with trailing comments moved: {stats['comments_moved_files']}")
        print(
            f"Files with identifiers formatted: {stats['files_with_identifiers']}")
        ...
        files_without_changes = max(
            0, total - stats['files_with_identifiers'] - stats['comments_moved_files'])
